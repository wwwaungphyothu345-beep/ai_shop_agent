...
def process_async_message(sender_id, customer_msg):
    try:
        # ၁။ Google Sheet ထဲက လက်ရှိ ပစ္စည်းစာရင်း Data ကို ဆွဲဖတ်ခြင်း
        sheet_data = call_google_script({"action": "read"})
        
        # ၂။ စာရင်းဖတ်ရရှိပါက ပစ္စည်းစာရင်းများကို တန်းစီပြီး စာသားထုတ်ပေးခြင်း
        if sheet_data and sheet_data.get("status") == "success" and "data" in sheet_data:
            ...
            # Google Script လင့်ခ်မှားယွင်းနေပါက ပြသမည့်စာသား
            reply_text = "⚠️ [Google Sheet Error]: Google Sheet မှ Data ဖတ်ယူ၍ မရနိုင်သေးပါ။ SCRIPT_URL လင့်ခ် သို့မဟုတ် Sheet Permissions ကို စစ်ဆေးပေးပါရှင်။"

        # ၃။ ကာစတန်မာထံသို့ Google Sheet ပါစာရင်းကို တိုက်ရိုက် စာပြန်ပေးခြင်း
...
def home():
    return "🚀 Live Dedicated Engine v250.0 [Stable Google Sheet Sync Mode] - Online!"
                ...
                for entry in data.get("entry", []):
                    for messaging_event in entry.get("messaging", []):
                        if messaging_event.get("message"):
                            ...
                            # argument အမှားကို သေချာပြန်ပြင်ဆင်ထားပါတယ်ဗျာ
